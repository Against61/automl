...
class IntentValidationSupport:
    ...
    def build_intent_validation_details(
        ...
        *,
        ...
        if reported_primary and intent.primary_metric_key:
            if reported_primary != intent.primary_metric_key.lower():
                validation_status = "failed"
                validation_reason = (
                    f"metrics artifact primary_metric_key='{reported_primary}' does not match "
                    f"inferred primary metric '{intent.primary_metric_key}'"
                )
                drift_detected = True
            else:
                validation_status = "passed"
                validation_reason = "metrics artifact primary_metric_key matches inferred intent"

        if not drift_detected and reported_family and intent.task_family != "generic":
            if reported_family != intent.task_family.lower():
                validation_status = "failed"
                validation_reason = (
                    f"metrics artifact task_family='{reported_family}' does not match "
                    f"inferred task family '{intent.task_family}'"
                )
                drift_detected = True
            elif validation_status == "skipped":
                validation_status = "passed"
                validation_reason = "metrics artifact task_family matches inferred intent"
